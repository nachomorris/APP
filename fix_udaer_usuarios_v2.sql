-- ============================================================
-- Fix v2: creación de usuarios UDAER (arregla el problema de
-- 'sin dueño'). Es 100% seguro volver a correr todo el archivo,
-- incluidas las partes 1 y 3 (no duplican nada).
--
-- Qué falló la vez pasada: en Supabase, pgcrypto (la extensión
-- que genera el hash de la contraseña) suele instalarse en el
-- esquema 'extensions', no en 'public'. Como el bloque anterior
-- llamaba a crypt()/gen_salt() sin decir en qué esquema buscar,
-- Postgres no las encontró, el bloque entero falló y se abortó
-- ANTES de crear ningún usuario ni asignar ningún dueño (por
-- eso las fichas quedaron marcadas como UDAER pero sin dueño:
-- esas 2 partes son sentencias aparte y sí se habían aplicado).
-- ============================================================

-- Asegura que pgcrypto esté disponible (no rompe nada si ya existe)
create extension if not exists pgcrypto with schema extensions;

-- 1) Altas: fichas que no existían en el catálogo -------------
-- (ya corrida la vez pasada; on conflict hace que sea segura repetirla)
...
declare
  ...
  v_user_id uuid;
begin
  -- cubre los dos esquemas donde Supabase puede tener pgcrypto instalado
  ...
  loop
    ...
    exception when others then
      -- si un mail puntual falla, lo avisa y sigue con el resto
      -- (se ve en la salida/'Logs' del SQL Editor de Supabase)
